...
export default function Home() {
    ...
    // 컴포넌트 마운트 시 오디오 요소 로드 및 로컬 스토리지 데이터 불러오기
    useEffect(() => {
        // 오디오 요소 로드
    ...
    const playAchievementSound = () => {
        if (achievementSoundRef.current) {
            achievementSoundRef.current.currentTime = 0; // 재생 위치를 처음으로 돌림 (중복 재생 시)
    ...
    useEffect(() => {
        ...
        ACHIEVEMENTS.forEach(achievement => {
            ...
            if (!achievementsUnlocked[achievement.id] && achievement.condition(winStats, tryCount, currentNet)) {
                ...
                playAchievementSound(); // 업적 달성 효과음 재생
    ...
    const getResultMessage = (): string | null => {
        ...
        // ⭐ 수정: 'bonus' 변수 이름 충돌 및 타입 불일치 해결
        const hasBonusMatch = myNumbers.includes(bonusNumber); // bonusNumber는 useState에서 관리하는 number 타입입니다.

        if (match === 6) return '🎉 축하합니다! 1등입니다!';
        if (match === 5 && hasBonusMatch) return '🥳 축하합니다! 2등입니다!'; // ⭐ 수정: 변경된 변수 이름 사용
    ...
    return (
        <div className={styles.flexWrapper}>
            ...
            {/* 효과음 재생을 위한 audio 태그 추가 (public 폴더에 achievement_sound.mp3가 있어야 함) */}
                    ...
                    <label>방식을 선택해주세요!
                        <select
                            ...
                            onChange={(e) => {
                                ...
                                setMode(selected);
                                resetManual(); // ✨ 모드 변경 시에도 수동/반자동 초기화 및 자동 추첨 중지
                                if (isAutoRunning) {
                                    stopAutoDraw();
                                    setIsAutoRunning(false);
                                }
